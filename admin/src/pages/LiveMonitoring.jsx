...
//   const socketRef = useRef(null);
//   const peers = useRef({});       // student_id -> RTCPeerConnection
//   const videoRefs = useRef({});   // student_id -> <video> element
//   const streams = useRef({});     // student_id -> MediaStream

//   const examId = "69d9ddf8e19f20efc27d676e";
...
//       // student-ready and violation carry studentId; guard centrally
//       const rawId = data.studentId;
//       const studentId = rawId != null ? String(rawId) : null;

//       // ── New student appeared ───────────────────────────────────────────
//       if (type === "student-ready") {
//         console.log("👨‍🎓 STUDENT READY:", studentId);

//         // Add to candidate list (idempotent)
//         setCandidates((prev) =>
//           prev.some((c) => String(c.id) === studentId)
//             ? prev
//             : [...prev, { id: studentId }]
...
//         // Small delay prevents race condition on page refresh
//         setTimeout(() => {
//           socket.send(JSON.stringify({ type: "watch-student", studentId }));
//         }, 500);
...
//         if (studentId) {
//           // Ensure card exists even if student-ready was missed
//           setCandidates((prev) =>
//             prev.some((c) => String(c.id) === studentId)
//               ? prev
//               : [...prev, { id: studentId }]
...
//             [studentId]: [
//               ...(prev[studentId] || []),
//               { code: data.violation, time: data.timestamp },
//             ],
//           }));
//         }
...
//       // All remaining message types (offer / answer / ice-candidate) require studentId
//       if (!studentId) return;

//       // ── Offer from server → create peer, send answer ───────────────────
//       if (type === "offer") {
//         console.log("📨 OFFER FROM:", studentId);

//         // Close stale peer if any
//         if (peers.current[studentId]) {
//           peers.current[studentId].close();
//           delete peers.current[studentId];
//         }
...
//         // Declare receive-only transceiver so the browser negotiates correctly
//         peer.addTransceiver("video", { direction: "recvonly" });
...
//         peer.ontrack = (event) => {
//           console.log("🎥 VIDEO RECEIVED FROM:", studentId);
//           const stream = event.streams[0];
//           streams.current[studentId] = stream;
...
//         // ICE candidates — send to server so it can forward to the student PC
//         peer.onicecandidate = (event) => {
//           if (event.candidate) {
//             socket.send(
//               JSON.stringify({
...
//     socket.onclose = () => console.log("❌ ADMIN DISCONNECTED");
