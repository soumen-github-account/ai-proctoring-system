...
# Single global relay instance — MUST be shared with consumers.py via import
relay = MediaRelay()

# student_id -> raw video track
# "_pc_{student_id}" -> RTCPeerConnection (kept alive to prevent GC)
student_tracks = {}
...
class VideoProcessor:
    ...
    async def handle_offer(self, offer):
        ...
        async def on_track(track):
            ...
            if track.kind == "video":
                # Store raw track — relay.subscribe() is called on-demand per viewer
                student_tracks[self.student_id] = track

                # CRITICAL: store the PC so it is NOT garbage collected
                # Without this the connection drops as soon as handle_offer() returns
                student_tracks[f"_pc_{self.student_id}"] = pc

                print("📡 STORED VIDEO FOR:", self.student_id)

                # Notify all admin sockets that this student is ready
                await self.channel_layer.group_send(
                    f"exam_{self.exam_id}",
                    {
                        "type": "signal",
                ...
                # Subscribe a fresh relay slot for the AI processing task
                asyncio.create_task(self.process_video(relay.subscribe(track)))
    ...
    async def process_video(self, track):
        ...
        while True:
            ...
            # Process every 10th frame to reduce AI load
            if frame_count % 10 != 0:
                continue
            ...
            for v in violations:
                ...
                # Rate-limit: same violation type can only fire once every 5 s
                if v in self.last_violation_time:
                    if now - self.last_violation_time[v] < 5:
                        continue
